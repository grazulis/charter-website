...
- The "petal" leaf shape is the brand's key graphic motif (the central negative space of the logo mark). **Always use the proper CDC petal: `border-radius: 0 47.6% 0 47.6% / 0 100% 0 100%` at a ~2.1:1 aspect ratio** — quarter-circle arcs (radius = petal height) with flat runs between them. This is the `.petal` class in `styles.css`; reuse it or copy its geometry. Never use the oval/lens form `border-radius: 0 100% 0 100%` on a square box — that is a different shape and is off-brand. For a petal turned 90°, swap the axes: `0 100% 0 100% / 0 47.6% 0 47.6%` at ~1:2.1.
